...
switch F
    ...
    case 'F2'%Schwefel's 2.22º¯Êý
        fobj = @F2;
        lb=-10;
        ub=10;
        dim=30;
        
    case 'F3'%Schwefel's 1.2º¯Êý
        fobj = @F3;
        lb=-100;
        ub=100;
        dim=30;
        
    case 'F4'%Schwefel's 2.21º¯Êý
        fobj = @F4;
        lb=-100;
        ub=100;
        dim=30;
...
function o = F1(x) %Sphereº¯Êý
o=sum((x.^2));
end

% F2

function o = F2(x)%Schwefel's 2.22º¯Êý
o=sum(abs(x))+prod(abs(x));
end

% F3

function o = F3(x)%Schwefel's 1.2º¯Êý
dim=size(x,2);
o=0;
for i=1:dim
    o=o+sum(x(1:i))^2;
...
function o = F4(x)%Schwefel's 2.21º¯Êý
o=max(abs(x));
end
